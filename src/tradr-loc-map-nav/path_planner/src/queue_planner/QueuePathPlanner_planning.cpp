...
void QueuePathPlanner::pathPlanningCallback(TaskSegmentPtr segment, TaskPtr task)
{
    ...
    { // start scope for locking mutex     
        ...
        segment->p_wall_pcl = wall_pcl_.makeShared(); // deep copy 
    ...
    if (utility_2d_flag_)
    {
        PathPlannerManager::cropTwoPcls((PathPlannerManager::CropBoxMethod)segment->crop_step, start, goal, traversability_pcl_, utility_2d_pcl_,
                    segment->p_traversability_pcl, segment->p_utility_2d_pcl);
    }
    else
    {
        PathPlannerManager::cropPcl((PathPlannerManager::CropBoxMethod)segment->crop_step, start, goal, traversability_pcl_, segment->p_traversability_pcl);
    }
