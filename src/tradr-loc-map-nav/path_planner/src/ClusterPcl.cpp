...
void ClusterPcl<PointT>::filterWall()
{
    ...
    sor.setInputCloud(pcl_wall_.makeShared());
    //sor.setLeafSize(0.05f, 0.05f, 0.05f);
    sor.setLeafSize(0.075f, 0.075f,0.075f); /// new setting
    sor.filter(new_pcl_wall);
    pcl::copyPointCloud(new_pcl_wall, pcl_wall_);
    ...
    kdtree_Wall.setInputCloud(pcl_wall_.makeShared());
    double radius = kFilterWallRadiusMin;
    for (int i = 0; i < pcl_wall_.size(); i++)
    {
    ...
    if (point_idx_filter_out_.size() > 0)
    {
        ...
        extract.setInputCloud(pcl_wall_.makeShared()); 
...
void ClusterPcl<PointT>::elaborateCluster()
{
    ...
    for (int i = 0; i < pcl_no_wall_.size(); i++)
    {
...
template<typename PointT>
void ClusterPcl<PointT>::setKdtree(const KdTreeIn& input_kdtree)
{
    kdtree_input_pcl_ = input_kdtree;
}
