...
PathManager::PathManager() : b_init_(false), b_jump_point_(false), b_end_(true), d_Ts_(0), d_vel_lin_(0), d_vel_ang_(0), d_yaw_last_(0), d_step_offset_(0), i_index_(0), d_estimated_time_(0), d_estimated_distance_(0)
{
}
...
void PathManager::init(double Ts, double vel, const nav_msgs::Path& path_in)
{
...
bool PathManager::step()
{
    ...
    while (!b_step_done && (i_index_ < path_in_.poses.size()))
    {
        ...
        else
        {
            ...
            {
                dist_step = fabs(vel * d_Ts_ - d_step_offset_); // get a shorter step 
...
/// Init
void PathManagerKdt::init(double Ts, double vel, const nav_msgs::Path& path_in)
{    
    boost::recursive_mutex::scoped_lock locker(mutex_);   
 
    ROS_INFO("PathManagerKdt::init()");
        
    PathManager::init(Ts, vel, path_in);
    ...
    for(size_t i=0; i<input_path_length;i++)
    {
        point.x     = path_in.poses[i].pose.position.x;
        point.y     = path_in.poses[i].pose.position.y;
        point.z     = path_in.poses[i].pose.position.z;
        point.label = i;
